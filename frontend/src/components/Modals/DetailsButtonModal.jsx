...
export const DetailsButtonModal = ({show, handleClose, cases, implementation}) => {
  const failedResults = []
  Array.from(implementation.cases.entries()).forEach(([seq, results]) => {
    ...
    for (let i = 0; i < results.length; i++) {
      ...
      if (state !== 'successful') {
        ...
        const borderClass = state === 'skipped' ? 'border-warning' : 'border-danger'
        failedResults.push(<DetailItem title={caseData.description} description={caseData.tests[i].description} message={message}
