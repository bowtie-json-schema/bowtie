...
public class BowtieSjf4jValidator {
    ...
    private final List<String> dialects = List.of("https://json-schema.org/draft/2020-12/schema");
    ...
    private void start(JsonObject jo) throws IOException {
        ...
        String fullName = "%s:%s".formatted(attributes.getValue("Implementation-Group"),
    ...
    private void run(JsonObject jo) {
        ...
        try {
            ...
            List<TestResult> results = runRequest.testCase().tests().stream()
                    .map(test -> new TestResult(schema.isValid(test.instance()))).toList();

            output.println(Sjf4j.toJsonString(new RunResponse(runRequest.seq(), results)));
        } catch (Exception e) {
