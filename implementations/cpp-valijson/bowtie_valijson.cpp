...
void freeDocument(const Document *adapter) {
    delete adapter;
}
...
int main() {
    ...
    for (string line; getline(cin, line);) {
        ...
        if (cmd == "start") {
            ...
            implementation.AddMember(
                    "issues",
                    "https://github.com/tristanpenman/valijson/issues",
                    allocator
            );
            ...
            if (!started) { throw runtime_error("Bowtie hasn't started!"); }
            ...
            for (auto& each : testCase["tests"].GetArray()) {
                ...
                result.AddMember("valid", validator.validate(schema, instance, NULL), allocator);
